...
const downloadFile = async (messageId, fileKey) => {
  ...
  const filePath = `./videos/${fileKey}.mp4`;
  try {
  ...
  server.route({
    ...
    handler: async (request, h) => {
      ...
      if (data.header.event_type === 'im.message.receive_v1') {
        ...
        if (data.event.message.message_type === 'media') {
      ...
      if (data.header.event_type === 'vc.meeting.all_meeting_ended_v1') {
        ...
        await sendMessageToUser(
          open_id,
          `Your recent meeting has been successfully processed, please download the recording from ${vidUrl} and upload it here`
        );
...
async function processMedia(mediaEvent, h) {
  ...
  const filePath = `./videos/${fileKey}.mp4`;
  const result = await transcribe(filePath, videoName, h);

  console.log('Transcription result:', result);

  // Send user a card asynchronously
  console.log(mediaEvent);
  const fileName = JSON.parse(mediaEvent.content).file_name.replace('.mp4', '');
  await sendCard(
    mediaEvent.chat_id,
    fileName,
    result.summary,
